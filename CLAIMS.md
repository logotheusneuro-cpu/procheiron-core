...
| **Signing custody in a single trust domain** | ed25519 signing gives real cross-actor non-repudiation only when actors hold **separate** private keys. In a single-OS-user deployment, keys on the shared filesystem are only as strong as their file permissions — separate-user / HSM / Sigstore-keyless custody is deployment guidance (roadmap 4). The hash-chain **tamper-evidence holds regardless** of key custody. |
