...
| **Signing not yet in the live deployment** | The authors' own live deployment migrated its audit log onto the tamper-evident chain on 2026-07-13 (legacy events rechained under the writer lock, writers now append chained and fail closed, the validator verifies the chain on every run, head anchored in a git commit — self-reported, single-team). **Signing is still not enabled there**: key custody (keys and registry out of the writer's write scope) is an open operational decision, so the live deployment currently has tamper-evidence, not authenticated provenance. |
| **Provenance is not correctness** | The gate proves *who* wrote and reviewed a record and that history was not rewritten — it cannot make the content true. A reviewer can approve a wrong fact and it becomes trusted; independent review catches self-dealing and forgery, not honest error. Wrongness is handled downstream — a traceable trail plus supersession/dispute to retire the record — not prevented at the gate. |
