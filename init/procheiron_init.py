...
def main() -> int:
    ...
    # Locate the adopter directory (same-level sibling: ../adopter relative to this file)
    here = Path(__file__).resolve().parent          # init/
    phase4 = here.parent                            # phase4/
    adopter = phase4 / "adopter"
    ...
    n_overwrote = sum(1 for v in results.values() if v == "overwritten")
    n_missing_src = sum(1 for v in results.values() if v == "source-missing")
    ...
    print(f"  python3 {root}/validate_minimal.py --root {root} --json")
